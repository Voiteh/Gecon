...
shared class IntersectionTest() extends BaseTest() {
	...
	additionalProviders => {object satisfies Provider{
		shared actual {Component*} components => {StringIntersectionResolver()};
	}};
	
	shared test 
	void shouldConvertToIntersectionContainingType(){
		assert(is InteresectionModel result=convertx.convert(matchingIntersection, `InteresectionModel`));
