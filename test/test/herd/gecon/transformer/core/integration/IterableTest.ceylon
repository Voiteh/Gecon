...
shared class IterableTest() extends CoreIntegration(){
	...
	void shouldConvertIterableToIterable(){
		value result=gecon.convert(testData.iterable.integer.stream,`{String*}`);
		 assertEquals{
		 	actual=result;
	...
	void shouldConvertIterableToList(){
		assert(is List<String> result=gecon.convert({1,2,3},`List<String>`)); 
	...
	void shouldConvertIterableToArray(){
		assert(is Array<String> result=gecon.convert({1,2,3}, `Array<String>`));
		assert(result.containsEvery({"1","2","3"}));
	}
	shared test
	void shouldConvertIterableToTouple(){
		assert(is [String,String,String] result=gecon.convert({1,2,3}, `[String,String,String]`));
	...
	void shouldConvertIterableToTupleWithDifferentTypes(){
		if(is Exception ex=gecon.convert({1,1,1}, `[String,Boolean,Float]`)){
			throw ex;
		}
		assert(is [String,Boolean,Float] result=gecon.convert({1,1,1}, `[String,Boolean,Float]`));
		assert(result.first=="1");
	...
	void shouldNotConvertIterableToToupleToFewArguments(){
		assert(is Conversion<>.Error error=gecon.convert({1,2,3}, `[String,String,String,String]`));
	}
	shared test
	void shouldNotConvertIterableToToupleToManyArguments(){
		assert(is Conversion<>.Error error=gecon.convert({1,2,3,4}, `[String,String,String]`));
	}
